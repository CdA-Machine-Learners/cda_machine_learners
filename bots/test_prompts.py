...
#
#     (I include this here as a reference since you have to write the template
#     in yourself.)
#
#
...
# tok(BOT_MESSAGE_1) + [EOS_ID] +
# …
# tok("[INST]") + tok(USER_MESSAGE_N) + tok("[/INST]") +
# tok(BOT_MESSAGE_N) + [EOS_ID]
#
#####
...
[Teacher] Yes! Interest in Stoicism climbed during the Roman period, and several stoic philosophers reported to the emperors. For example, during the Imperial era, in the late 1st century BCE, Arius Didymus was appointed to the court of Emperor Augustus. A generation later, Seneca served as advisor to Nero."""
        },
        {
            'role':'user',
            'content': f"""
That's great, good job imagining that dialog. It sticks to the given details well. This is critical, ie to ground the discussion in facts from the passage. For our next passage, if you feel it's justified, allow the Student to ask more open ended questions, and allow the Teacher to answer at length, like a lecture. Here is the next passage for you to convert to dialog:
...
{passage}"""
        },
        {
            'role':'assistant',
            'content': f"""
I see. I will focus on having the Student ask more broad questions, and the Teacher will answer open ended questions at great length, and in great detail, perhaps like part of a lecture. Above all we should ground the discussion on the original passage and not infer too much. Now let's generate a dialog for your new passage:
...
try:
    book_loaded
    print('book already loaded')
except:
    print('loading book')
    book_path = os.path.expanduser('~/_/bots/data/mormon/Jesus the Christ - James E Talmage.txt')
...
def generate_batch(batch: List[str]):
    print(f'Computing batch size={len(batch)}', end='')
    ...
    print(f', time={elapsed_time}, new_toks={num_toks}, tok/s={toks_per_second}')
    ...
    try:
        ...
        with open(output_path, 'a') as f:
            for passage, out_tok in zip(batch, out_toks):
                ...
                print('----------')
                print(f'tok/s: {tok_per_s}')
                print('PASSAGE:')
                print(passage[:100], end='\n...\n')
                print(passage[-100:])
                print('DIALOG:')
                print(dialog)
                if len(dialog) >= 2:
