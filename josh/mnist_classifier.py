...
data_path = '/home/mobius/_/beta/experiments/data'
...
# ys = np.zeros((15, 10)) # onehot vecs
# for i, (x, y) in enumerate(train_ds[:15]):
#     # plot one handwritten digit
#     plt.subplot(4, 4, i+1)
#     plt.imshow(x.reshape((28, 28)).detach().cpu().numpy())
#     ys[i] = y.detach().numpy()
# # plot all onehot labels
# plt.subplot(4, 4, 16)
# plt.imshow(ys)
# plt.show()
...
# for i in range(16):
#     a = model.w[0].weight[i].reshape((28, 28)).detach().cpu().numpy()
#     plt.subplot(4, 4, i+1)
#     plt.imshow(a)
# plt.show()
